...
{
    public class FullBodyAvatarMocap : TrackingEventHandler
    {
        ...
        [HideInInspector]
        public CoreHolisticTrackingSolution coreHolisticSolution;
        ...
        {
            //here we are handing pose data only. if you want to face and hand finger tracking also add similar handlers for them.
            coreHolisticSolution = (CoreHolisticTrackingSolution)solution;
            if (!isTracking())
            {
                return;
            }

            //how to access face landmarks:
            //coreHolisticSolution.faceLandmarks.Landmark[0]

            //how to access hand landmarks:
            //coreHolisticSolution.leftHandLandmarks.Landmark[0]
            //coreHolisticSolution.rightHandLandmarks.Landmark[0]

            for (int i = 0; i < _numberOfBones; i++)
            {
                var position = new Vector3(-coreHolisticSolution.poseWorldLandmarks.Landmark[i].X, -coreHolisticSolution.poseWorldLandmarks.Landmark[i].Y, -coreHolisticSolution.poseWorldLandmarks.Landmark[i].Z);
                if (i == (int)Body.NOSE) position.z *= .3f;
                else position.z *= .9f;
                bones[i].visibility = coreHolisticSolution.poseWorldLandmarks.Landmark[i].Visibility;
                bones[i].transform.position = position;
            }

            if (coreHolisticSolution.poseRoi != null) _roiWidth = coreHolisticSolution.poseRoi.Width;
        ...
        private void Move()
        {
            var leftHipPosition = screen.rect.GetPoint(coreHolisticSolution.poseLandmarks.Landmark[(int)Body.LEFT_HIP], 0, false);
            var rightHipPosition = screen.rect.GetPoint(coreHolisticSolution.poseLandmarks.Landmark[(int)Body.RIGHT_HIP], 0, false);
        ...
        {
            if (coreHolisticSolution==null||coreHolisticSolution.poseWorldLandmarks == null)
            {
                return false;
            }
            return true;
