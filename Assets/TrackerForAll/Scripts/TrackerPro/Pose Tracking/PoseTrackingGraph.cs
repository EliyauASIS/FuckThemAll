...
namespace TrackerPro.Unity.PoseTracking
{
    public class PoseTrackingGraph : GraphRunner
    {
        ...
        [Header("Choose model complexity. Higher complexity may increase accuracy but decrease performance.")]
        ...
        public float _minDetectionConfidence = .5f;
        ...
        public float _minTrackingConfidence = .5f;
        ...
        protected override Status ConfigureCalculatorGraph(CalculatorGraphConfig config)
        {
            ...
            //Should replce by posesolution orginal code
        ...
        private WaitForResult WaitForPoseLandmarkModel()
        {
            switch (modelComplexity)
            {
                ...
                default: throw new InternalException($"Invalid model complexity1: {modelComplexity}");
            }
        }
        private WaitForResult WaitForHandLandmarkModel()
        {
            switch (modelComplexity)
            {
                ...
                case ModelComplexity.Heavy: return WaitForAsset("hand_landmark_full.bytes");
                default: throw new InternalException($"Invalid model complexity2: {modelComplexity}");
            }
        }
        private WaitForResult WaitForPalmDetectionModel()
        {
            switch (modelComplexity)
            {
                ...
                case ModelComplexity.Heavy: return WaitForAsset("palm_detection_full.bytes");
                default: throw new InternalException($"Invalid model complexity3: {modelComplexity}");
            }
        }
